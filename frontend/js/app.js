...
async function boot() {
  const [geo, risk, style] = await Promise.all([
    fetch("data/wilayas.geojson").then((r) => r.json()),
    loadJSON("data/risk.json", "data/mock_risk.json"),
    darkBasemapStyle(),
  ]);
...
async function loadSecondary() {
  try {
    const fires = await loadJSON("data/fires.json", "data/mock_fires.json");
    S.fires = fires;
    renderFires();
    renderFireStats();
  } catch (err) {
  ...
  try {
    const exposure = await fetch("data/exposure.json").then((r) => r.json());
    S.exposure = exposure;
  ...
  try {
    const assets = await fetch("data/assets_map.json").then((r) => r.json());
