...
async function loadRisk() {
  try {
    return await getJSON("/api/v1/risk", dataURL("risk.json"));
  } catch (e) {
    return getJSON(null, dataURL("risk.json"));
  }
}

async function loadFires() {
  try {
    return await getJSON("/api/v1/fires", dataURL("fires.json"));
  } catch (e) {
    return getJSON(null, dataURL("fires.json"));
  }
}

const loadExposure = () => getJSON("/api/v1/exposure", dataURL("exposure.json"));
const loadAssets = () => getJSON("/api/v1/assets", dataURL("assets_map.json"));
const loadWilayaGeo = () => getJSON(null, dataURL("wilayas.geojson"));
